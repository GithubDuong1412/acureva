        ...
        $this
            ...
            ->addWrapper(
                ...
                function (CheckoutForm $form) use ($token, $model): void {
                    ...
                    $form
                        ->when(! $isMobile, function (CheckoutForm $form) use ($model, $discountFormHtml): void {
                            $form->addWrapper(
                                'right_column_wrapper',
                                '<div class="col-lg-5 col-md-6 order-2 checkout-order-info">',
                                '</div>',
                                function (CheckoutForm $form) use ($discountFormHtml, $model): void {
                                    $form
                                        // ->addWrapper(
                                            ...
                                            function (CheckoutForm $form) use ($model): void {
                                                $form->add(
                                                    ...
                                                    HtmlFieldOption::make()->content(
                                                        view('plugins/ecommerce::orders.checkout.products', [
                                                            'cartItems' => $model['cartItems'] ?? collect(),
                                                            'products' => $model['products'] ?? collect(),
                                                        ])
                                                    )
                                                );
                                            }
                        ...
                        ->addWrapper(
                            ...
                            '</div>',
                            function (CheckoutForm $form) use ($isMobile, $discountFormHtml, $token, $model): void {
                                $form
                                    ->addWrapper(
                                    ...
                                    ->when($isMobile, function (CheckoutForm $form) use ($discountFormHtml, $model): void {
                                        $form
                                            // ->addWrapper(
