...
(function($) {
    ...
    const SizeGuideTableBuilder = {
        ...
        addColumn: function(e) {
            ...
            // Get header options from an existing select, or fall back to template options
            let optionsHtml = '';
            const $existingSelect = $('.column-header-input').first();
            const $templateSelect = $('#size-guide-header-options-template');

            const existingHasOptions = $existingSelect.length
                && $existingSelect.is('select')
                && $existingSelect.find('option').length > 0;

            const $optionsSource = existingHasOptions ? $existingSelect : $templateSelect;

            $optionsSource.find('option').each(function() {
